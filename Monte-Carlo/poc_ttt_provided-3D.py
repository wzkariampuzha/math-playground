...
class TTTBoard:
    ...
    def __init__(self, dim, reverse = False, board = None):
        ...
        if board == None:
            ...
            self._board = [[[EMPTY for dummycol in range(dim)] 
                           ...
                           for dummyz in range(dim)]
        else:
            # Copy board grid
            self._board = [[[board[row][col][dep] for col in range(dim)] 
                           for row in range(dim)] for z in range(dim)]
    ...
    def __str__(self):
        ...
        for row in range(self._dim):
            for col in range(self._dim):
                for z in range(self._dim):
                    rep += STRMAP[self._board[row][col][z]]
                    if col == self._dim - 1:
                        rep += "\n"
                    else:
                        rep += " | "
    ...
    def check_win(self):
        ...
        # check all lines
        for line in lines:
            #This finds out if there is only a unique number in a line and not EMPTY
            #then returns identity of the first entry
